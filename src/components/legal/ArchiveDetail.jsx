...
import { Search, FileText, Scale, Gavel, BookOpen, PlusCircle, ScanLine, FileImage, Archive } from 'lucide-react';
...
const ArchiveDetail = () => {
  ...
  const currentArchiveData = mockArchiveData[archiveType] || [];

  const getTitle = (type) => {
    switch (type) {
      case "meeting-minutes": return "Meeting Minutes";
      case "agreements": return "Agreements";
      case "legislative-opinions": return "Legislative Opinions";
      case "legal-opinions": return "Legal Opinions";
  ...
  const getIcon = (type) => {
    switch (type) {
      case "meeting-minutes": return FileText;
      case "agreements": return Scale;
      case "legislative-opinions": return Gavel;
      case "legal-opinions": return BookOpen;
      case "settlements": return Search;
      default: return Archive;
    }
  };

  const currentTitle = getTitle(archiveType);
  const CurrentIcon = getIcon(archiveType);
      ...
      setTimeout(() => {
        ...
        if (step.progress === 100) {
          setTimeout(() => {
            ...
            alert(
              `Paper ${currentTitle} successfully converted to digital format using OCR!`
            );
          }, 1000);
        }
      }, (index + 1) * 1500);
  ...
  const handleModalSave = () => {
    ...
    console.log(`Saving ${archiveType} data:`, modalData);
    setIsModalOpen(false);
    alert(`New ${currentTitle} added/updated successfully! (Check console for data)`);
  };
  ...
  return (
    <div className="p-6">
      <h1 className="text-3xl font-bold text-gray-900 mb-8 flex items-center">
        <CurrentIcon className="h-8 w-8 mr-3 text-blue-600" />
        {currentTitle} Archiving
      </h1>
      <p className="text-gray-600 mb-8">
        This section manages {currentTitle.toLowerCase()} documents. You can add new digital documents or convert paper documents using OCR.
      </p>

      <div className="flex space-x-4 mb-6">
        <Button onClick={handleAddClick}>
          <PlusCircle className="h-4 w-4 mr-2" /> Add New {currentTitle}
        </Button>
        <Button variant="outline" onClick={handlePaperClick}>
          <FileImage className="h-4 w-4 mr-2" /> Paper {currentTitle}
        </Button>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              {currentArchiveData.length > 0 && Object.keys(currentArchiveData[0]).map((key) => (
      ...
      {currentArchiveData.length === 0 && (
        <p className="text-center text-gray-500 py-4">No {currentTitle.toLowerCase()} found.</p>
      )}

      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="sm:max-w-[600px]">
          <DialogHeader>
            <DialogTitle>{modalData.id ? `Edit ${currentTitle}` : `Add New ${currentTitle}`}</DialogTitle>
            <DialogDescription>
              Fill in the details for the {currentTitle.toLowerCase()} document.
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
            {renderModalContent()}
      ...
      <Dialog open={isOcrModalOpen} onOpenChange={setIsOcrModalOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Processing paper {currentTitle.toLowerCase()} using OCR technology...
            </DialogDescription>
          </DialogHeader>
          <div className="py-6">
